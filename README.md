...
The current test waveform is a simple unmodulated **CW burst** containing approximately four million samples at a sampling rate of **61.44 MS/s**. The architecture itself is waveform-independent: the TX payload may contain an arbitrary pulse, coded sequence, modulation pattern, or other digitally generated waveform. More advanced waveforms and pulse-compression techniques are planned for longer-range detection. Current short-range experiments use an RF bandwidth of **20 MHz**.

This relatively wide bandwidth supports short pulses and improved range resolution, although it also increases the integrated receiver noise. At short range, this trade-off is acceptable because the reflected signals are comparatively strong.
...
## 2. FPGA / SoC Architecture

![Simplified Zynq radar dataflow](img/04_zynq_dataflow.svg)

The Vivado design separates the deterministic sample-processing path in the **Programmable Logic** from buffer management, system configuration and USB transport in the **Processing System**.

- **TX path:** the PS loads the transmit waveform into dual-port BRAM through an AXI BRAM controller. The TX player reads the payload in the RF clock domain, generates I/Q samples, and the DDR output block drives the AD9363 digital TX interface.
- **RX path:** the AD9363 DDR input is converted into a 32-bit AXI4-Stream. The packing block also injects the current hardware frame ID and antenna angle into the reserved metadata region. An asynchronous AXI-Stream FIFO crosses into the DMA clock domain and isolates the acquisition path from temporary downstream backpressure.
- **DMA and memory:** AXI DMA operates in S2MM scatter-gather mode and writes completed stream blocks to PS DDR through the Zynq high-performance AXI port. The PS then forwards the buffers to the host through USB bulk transfers.
- **Timing and metadata:** the capture controller sequences single and continuous acquisitions and applies the configured interframe interval. The frame detector tracks completed AXI-Stream frames and generates the hardware frame ID. A dedicated UART receiver captures the antenna azimuth value for insertion into each frame.
- **Error monitoring:** DMA completion and PL overrun conditions are returned to the PS through interrupts. The host independently verifies transport continuity using the frame ID embedded in every frame.

[View the complete Vivado block design (PDF)](design_1.pdf)

---

## 3. Key Technical Characteristics

The current prototype combines a deterministic FPGA acquisition path with software-managed streaming and host-side processing.

| Parameter | Current value |
|---|---:|
| FPGA / SoC | Xilinx Zynq-7020 |
| RF transceiver | AD9363 on OpenSDR |
| I/Q sampling rate | 61.44 MS/s |
| Configured RF bandwidth | 20 MHz |
| RX stream | 32-bit AXI4-Stream |
| Clock-domain crossing | Asynchronous AXI-Stream FIFO |
| Memory transfer | AXI DMA S2MM, scatter-gather |
| System memory path | Zynq HP AXI port to PS DDR |
| Host interface | USB bulk, approximately 150 Mbit/s |
| Acquisition modes | Single frame and continuous streaming |
| Angle synchronization | UART angle input embedded into every frame |
| Host software | Python |
| Visualization | A-scope and B-scope |
